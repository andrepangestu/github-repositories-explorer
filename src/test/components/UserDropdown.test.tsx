...
describe('UserDropdown Component', () => {
  ...
  it('shows chevron down icon when collapsed', () => {
    ...
    // Check that chevron down icon is present
    const chevronDown = within(button).getByTestId('chevron-down') || 
  ...
  it('shows chevron up icon when expanded', () => {
    ...
    // Check that chevron up icon is present
    const chevronUp = within(button).getByTestId('chevron-up') || 
  ...
  it('shows empty state when no repositories and not loading', () => {
    ...
    // Should show some indication that there are no repositories
    const repoSection = document.getElementById(`repos-${mockUser.id}`)
    expect(repoSection).toBeInTheDocument()
    expect(screen.queryByText('Loading repositories...')).not.toBeInTheDocument()
  })
  ...
  it('maintains proper button state during loading', () => {
    ...
    const button = screen.getByRole('button')
    expect(button).toHaveAttribute('aria-expanded', 'true')
    expect(button).not.toBeDisabled() // Button should remain clickable
  })
